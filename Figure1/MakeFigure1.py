...
DifferentWaterAmount = [-5.00,-4.00,-3.00,-2.00, -1.75, -1.50, -1.25, -1.00]
...
for Counter, WaterAmount in enumerate(DifferentWaterAmount):
    ...
    if WaterAmount>=-1.75:
        ax2.plot(BinnedWavelength, BinnedModel, color=colorList[Counter], lw=2, label=Label)    
...
plt.tight_layout()
plt.savefig("DifferentWaterAbundance_Square_LongerRange.png")
plt.savefig("DifferentWaterAbundance_Square_LongerRange.pdf")
plt.close()
